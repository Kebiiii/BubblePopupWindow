package com.kebiiii.bubblepopup;
...
public class BubblePopupWindow extends PopupWindow {
    ...
    public void show(View parent, int gravity, float bubbleOffset,int offx,int offy) {
        BubbleRelativeLayout.BubbleLegOrientation orientation = BubbleRelativeLayout.BubbleLegOrientation.LEFT;
        if (!this.isShowing()) {
            switch (gravity) {
                case Gravity.BOTTOM:
            ...
            switch (gravity) {
                case Gravity.BOTTOM:
                    showAsDropDown(parent, offx, offy, Gravity.NO_GRAVITY );
                    break;
                case Gravity.TOP:
                    showAtLocation(parent, Gravity.NO_GRAVITY, location[0] + offx, location[1] - getMeasureHeight() + offy);
                    break;
                case Gravity.RIGHT:
                    showAtLocation(parent, Gravity.NO_GRAVITY, location[0] + parent.getWidth() + offx, location[1] - (parent.getHeight() / 2) + offy);
                    break;
                case Gravity.LEFT:
                    showAtLocation(parent, Gravity.NO_GRAVITY, location[0] - getMeasuredWidth() + offx, location[1] - (parent.getHeight() / 2) + offy);
                    break;
                default:
                    break;
            }
